<?php
function get_web_attr($web,$conn){
	$result="";
    $select_type=mysqli_query($conn,"SELECT type FROM vt_web WHERE web='$web'");
    $row=mysqli_fetch_array($select_type);
    if(mysqli_num_rows($select_type)==1){
    	$result=$row["type"];
    }
    return $result;
}
function get_same_substring($link,$test){//bug XXXXXXX.sameweb.videoid
    $copy_test=$test;
    $add=0;
    $temp_link=$link;
...
function get_web_cont($web,$conn){
    $result=array();
    $select_type=mysqli_query($conn,"SELECT content,offset,from_link,from_offset,num FROM vt_web WHERE web='$web'");
    $row=mysqli_fetch_array($select_type);
    if(mysqli_num_rows($select_type)==1){
        $result["content"]=$row["content"];
        $result["offset"]=$row["offset"];
        $result["from_link"]=$row["from_link"];
        $result["from_offset"]=$row["from_offset"];
        $result["num"]=$row["num"];
    }
    return $result;    
}
