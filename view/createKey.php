        ...
        <?php
            ...
            if($type=="video"){
                ...
                if(stristr($link,$web_con["from_link"])){
                    ...
                    $num_w=$web_con["num"];
                    if($web_con["from_link"]=="?" && isset($same_temp[1])){
                        $same_temp1="?".$same_temp[1];
                    }
                    if($web_con["from_link"]!="?" && isset($same_temp[$num_w])){
                        $same_temp1=$same_temp[$num_w];
                    }
                    if($web_con["from_offset"]!=null){
                        $same_temp2=explode($web_con["from_offset"],$same_temp1);
                        $same_link=$same_temp2[0];
                    }else{
                        $same_link=$same_temp1;
                    }    
                }
                ?>
                <input style="display:none;" id="input_content" value="<?php echo $web_con["content"].$same_link.$web_con["offset"];?>"/>
                <li style="display:none;" id="select_type" name="select_type" value="<?php echo "1";?>">视频</li>
                <!--<object class="link_input" width="440" height="356" data="<?php echo $web_con["content"].$same_link.$web_con["offset"];?>"></object>-->
                <iframe class="link_input" style="border:none;" width="440" height="356" src="<?php echo $web_con["content"].$same_link.$web_con["offset"];?>"></iframe>
                <?php
            }
        }
        ?>
